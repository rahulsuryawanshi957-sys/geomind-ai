...
interface CalcDef { id: string; label: string; category: string; fields: FieldDef[] }

const CATEGORY_ORDER = ['Soil Parameters', 'Foundation / Bearing Capacity', 'Settlement', 'Lateral Earth Pressure', 'Rock']

const CALC_DEFS: CalcDef[] = [
  { id: 'spt_correction', category: 'Soil Parameters', label: 'SPT N-value Correction', fields: [
  ...
  { id: 'bearing_capacity_terzaghi', category: 'Foundation / Bearing Capacity', label: 'Bearing Capacity (Terzaghi)', fields: [
  ...
  { id: 'bearing_capacity_is6403_shear', category: 'Foundation / Bearing Capacity', label: 'SBC — IS:6403 Shear Method', fields: [
  ...
  { id: 'settlement_sbc_is8009_noncohesive', category: 'Settlement', label: 'SBC — IS:8009 Settlement (Granular)', fields: [
    { key: 'length_m', label: 'Footing length L', unit: 'm', default: 3 },
    { key: 'width_m', label: 'Footing width B', unit: 'm', default: 3 },
    { key: 'depth_m', label: 'Depth of footing D', unit: 'm', default: 3 },
  ...
  { id: 'settlement_sbc_is8009_cohesive', category: 'Settlement', label: 'SBC — IS:8009 Settlement (Clay)', fields: [
    { key: 'length_m', label: 'Footing length L', unit: 'm', default: 3 },
    { key: 'width_m', label: 'Footing width B', unit: 'm', default: 3 },
    { key: 'depth_m', label: 'Depth of footing D', unit: 'm', default: 3 },
  ...
  { id: 'immediate_settlement', category: 'Settlement', label: 'Immediate (Elastic) Settlement', fields: [
  ...
  { id: 'consolidation_settlement', category: 'Settlement', label: 'Consolidation Settlement', fields: [
  ...
  { id: 'earth_pressure_rankine', category: 'Lateral Earth Pressure', label: 'Rankine Earth Pressure', fields: [
    { key: 'gamma_kn_m3', label: 'Unit weight γ', unit: 'kN/m³', default: 18 },
...
const PLANNED_CATEGORIES: Record<string, string> = {
  safe_bearing_capacity: 'Foundation / Bearing Capacity',
  raft_foundation: 'Foundation / Bearing Capacity',
  isolated_footing: 'Foundation / Bearing Capacity',
  pile_capacity: 'Foundation / Bearing Capacity',
  group_efficiency: 'Foundation / Bearing Capacity',
  lateral_pile: 'Foundation / Bearing Capacity',
  modulus_subgrade_reaction: 'Settlement',
  plate_load_test: 'Soil Parameters',
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        <div className="lg:w-64 shrink-0 space-y-1">
          {CATEGORY_ORDER.map((cat) => {
            const implementedInCat = CALC_DEFS.filter((c) => c.category === cat)
            const plannedInCat = planned.filter((p) => (PLANNED_CATEGORIES[p] || 'Other') === cat)
            if (implementedInCat.length === 0 && plannedInCat.length === 0) return null
            return (
              <div key={cat} className="mb-3">
                <div className="text-[10px] uppercase tracking-wider text-slate-500 px-3 mb-1">{cat}</div>
                {implementedInCat.map((c) => (
                ...
                {plannedInCat.map((p) => (
                  <div key={p} className="w-full text-left px-3 py-1.5 rounded-xl text-xs text-slate-500 flex items-center gap-2">
                    <Construction size={12} /> {p.replace(/_/g, ' ')} <span className="gm-badge bg-white/[0.06] text-slate-500 ml-auto">Soon</span>
                  </div>
                ))}
              </div>
            )
