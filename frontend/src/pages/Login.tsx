...
// app's theme-variable accent -- the login page's premium navy+orange identity should
// look the same regardless of the in-app light/dark toggle. Uses `brand-orange`
// (#FF8A00, tailwind.config.js) rather than raw hex so it stays in sync with the
...
export default function Login({ onLoggedIn }: { onLoggedIn: () => void }) {
  ...
  return (
    // #020817 -- exact background hex from Raahi's 8 Aug 2026 "final direction" brief
    // (was #050B14, a very close but not identical navy, in the previous round).
    <div className="relative min-h-screen bg-[#020817] overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center px-4 pt-8 pb-14 sm:pt-12 sm:pb-16">
        <div className="w-full max-w-6xl mx-auto flex flex-col gap-12 md:gap-16">
          {/* Branding (left/center) + login card (right on desktop) -- side by side again
              per this round's brief ("LEFT/CENTER: branding. RIGHT: login card."),
              reverting the previous round's stacked-below-branding layout. */}
          <div className="flex flex-col xl:flex-row items-center xl:items-start gap-10 xl:gap-16">
            ...
            <div className="w-full xl:flex-1 flex flex-col items-center xl:items-start text-center xl:text-left">
              ...
              {/* Capability trust-row: hidden on mobile (per brief's mobile content order,
                  which goes straight from tagline to the login card) -- shown from md up. */}
              <div className="hidden md:grid mt-9 grid-cols-4 gap-6 w-full max-w-md xl:max-w-none">
            ...
            {/* Right: login card. Mobile: ~90% of screen width per brief ("85-92%"),
                not a fixed narrow max-w-sm. */}
            <div className="w-[90%] sm:w-full sm:max-w-sm xl:w-[400px] xl:shrink-0 xl:pt-2">
              <form
                onSubmit={handleSubmit}
                className="rounded-2xl border border-white/10 bg-[#0D1B2A]/85 backdrop-blur-xl p-6 sm:p-7 shadow-[0_25px_70px_-20px_rgba(0,0,0,0.65)]"
              >
                ...
                <div className="space-y-4">
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-brand-orange to-[#FFAA2B] hover:brightness-110 text-white font-semibold py-[11px] shadow-[0_10px_30px_-10px_rgba(255,138,0,0.5)] transition disabled:opacity-60 disabled:cursor-not-allowed"
                  >
          ...
          {/* Capability strip -- always visible, mobile included */}
